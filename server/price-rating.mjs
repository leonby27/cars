...
//    молча.
//
// 5. Пробег влияет не только на состав набора, но и на саму планку. Одного условия
//    «похожий пробег» мало: допуск широкий, и внутри него машина с 90 тысячами
//    сравнивалась бы с машинами по 60 как с равными. Поэтому по самому набору
//    считается, сколько цена теряет на километре, и планка сдвигается под пробег
//    этой машины: набор делится пополам по пробегу, у каждой половины берётся
//    типичная цена и типичный пробег, а разница между половинами и даёт цену
//    километра. Способ грубый, зато считается по нашим же ценам и ничего не
//    выдумывает. Он применяется только когда половины различимы (пробеги
//    расходятся хотя бы на седьмую часть) и когда цена с пробегом действительно
//    падает; сдвиг ограничен седьмой частью цены, чтобы редкая кривая выборка не
//    уводила планку в сторону.
//
...
export function priceRatingFrom(car, rows) {
  ...
  // Планку считаем отдельно для каждого режима цен: наклон по пробегу у цен с
  // пошлиной и без неё разный, потому что и сами цены разные.
  const mode = (priceOf, ownUsd) => {
    const prices = chosen.items.map(priceOf);
    const medianUsd = median(prices);
    const expected = expectedPrice(medianUsd, mileageMedian, mileage, mileagePriceSlope(chosen.items, priceOf));
    return {
      medianUsd,
      expectedUsd:expected.usd,
      mileageAdjusted:expected.adjusted,
