...
//
// 2. Сначала сравниваем с той же моделью и тем же типом силовой установки, затем —
//    с тем же годом. Электромобиль, гибрид и машина с ДВС одной модели могут иметь
//    совсем разные батареи, таможенные платежи и цены; смешивать их нельзя. Цена
//    также падает с возрастом, и в общем наборе любая машина 2021 года оказалась бы
//    «дешёвой», а любая 2024-го — «дорогой», ничего этим не сказав.
//
 ...
 *
 * Всё это внутри той же модели и того же типа силовой установки; последняя ступень
 * допускает другие годы. Условие, которого у машины нет — например батарея у машины
 * с мотором, — просто не работает и набор не сужает.
 *
...
export function chooseComparables(car, rows) {
  ...
  const others = rows.filter((item) => String(item.id) !== String(car?.id)
    && (!type || String(item.type || "").trim() === type));
