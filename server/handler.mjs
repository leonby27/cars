...
// 28.08.2026 окно продлено с 300/600 до 900/86400. Замер на боевом сервере: на прогретом
// кэше все запросы главной укладываются в 0,24 с, мимо кэша те же — около 4 с. Прежнее
// окно жило пятнадцать минут, и на молодом сайте с редкими заходами запись успевала
// пропасть между посетителями: холодный ответ доставался не одному из тысячи, а многим.
// Сутки `stale-while-revalidate` значат не «показываем вчерашнее», а «не заставляем
