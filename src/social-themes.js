...
// (src/analytics.css): отступ от края со всех сторон, а снизу ещё и полоса,
// отданная логотипу, чтобы надпись на него не налезала. Место закреплено за темой:
// тема должна узнаваться в ленте по одному взгляду, поэтому её заголовок всегда на
...
const NBSP = "\u00a0";
const SHORT_WORDS = /(^|[\s(«])([А-Яа-яЁёA-Za-z]{1,2}|для|под|при|или)\s+/g;
...
export function typeset(text) {
  return String(text || "")
    ...
    // Короткие слова тянут за собой следующее: «до 25 000 $», «на электротяге».
    .replace(SHORT_WORDS, `$1$2${NBSP}`)
    .replace(TRAILING_SHORT, NBSP);
}
const priceUsd = (car) => {
...
export function tileHeadline(theme, pick, loaded, round = 0) {
  ...
  return typeset(`${modelName(loaded)}, ${loaded.year}${price ? ` — ${price} под ключ` : ""}`);
}
