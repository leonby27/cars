...
function Header({ navigate, favoritesCount, path, currency, setCurrency }) {
  ...
  return <header className="site-header"><div className="header-inner">
    ...
    <nav className="desktop-nav" aria-label="Основная навигация"><button className={catalogActive ? "active" : ""} aria-current={catalogActive ? "page" : undefined} onClick={() => navigate("/catalog")}>Автомобили</button><button className={path === "/how-it-works" ? "active" : ""} aria-current={path === "/how-it-works" ? "page" : undefined} onClick={() => navigate("/how-it-works")}>Как это работает</button><button className={path === "/about" ? "active" : ""} aria-current={path === "/about" ? "page" : undefined} onClick={() => navigate("/about")}>О сервисе</button></nav>
...
function FeaturedCard({ car, onClick }) {
  ...
  return <article className="featured-card" onClick={onClick} onKeyDown={(e) => (e.key === "Enter" || e.key === " ") && onClick()} tabIndex="0" role="button" aria-label={`Открыть ${car.title}`}><HoverImagePreview car={car} className="featured-image"/><div className="featured-body"><h3>{car.title}</h3><p>{number(car.mileage)} км · {car.type} · {car.drive}</p><div className="featured-price"><strong>≈ {money(price.totalUsd, currency)}</strong><small>под ключ до Минска</small></div></div></article>;
}
...
function FilterPanel({ filters, setFilters, resultCount, brands, models, bodyTypes, drives, availability }) {
  ...
  const submit = <button className="primary filter-submit"><MagnifyingGlass size={19} weight="bold"/>Показать {resultCount} авто</button>;
  return <section className="filter-panel unified-search-panel"><div className="type-tabs">{["Все","Электромобили","Гибриды"].map((item) => <button type="button" key={item} className={selectedType === item ? "active" : ""} onClick={() => selectType(item)}>{item}</button>)}</div><div className="filter-primary-row unified-filter-primary"><SelectField label="Марка" value={filters.brand} onChange={changeBrand} options={["Все марки", ...brands]} searchable/><SelectField label="Модель" value={filters.model} onChange={update("model")} options={models} searchable disabled={filters.brand === "Все марки"}/><SelectField label="Год выпуска" value={filters.year} onChange={update("year")} options={["от 2022","от 2023","от 2024"]}/><SelectField label="Цена до Минска" value={filters.price} onChange={update("price")} options={["до $40 000","до $30 000","до $25 000"]} formatOption={(value) => priceLimitLabel(value, currency)}/>{!moreFiltersOpen && submit}</div>{moreFiltersOpen && <><div className="filter-extra-row" id="catalog-extra-filters"><SelectField label="Кузов" value={filters.bodyType} onChange={(value) => setFilters((old) => ({ ...old, bodyType:value, model:"Все модели" }))} options={bodyTypes}/><SelectField label="Пробег" value={filters.mileage} onChange={update("mileage")} options={["до 50 000 км","до 30 000 км","до 15 000 км"]}/>{Number(availability.drive) > 0 && <SelectField label="Привод" value={filters.drive} onChange={update("drive")} options={drives}/>} {Number(availability.owners) > 0 && <SelectField label="Владельцы" value={filters.owners} onChange={update("owners")} options={ownerOptions}/>} {Number(availability.claims) > 0 && <SelectField label="История" value={filters.history} onChange={update("history")} options={historyOptions}/>}</div><div className="expanded-filter-submit">{submit}</div></>}<button type="button" className="more-filters-toggle" aria-expanded={moreFiltersOpen} aria-controls="catalog-extra-filters" onClick={() => setMoreFiltersOpen((open) => !open)}><SlidersHorizontal size={17}/>{moreFiltersOpen ? "Скрыть фильтры" : "Ещё фильтры"}<CaretDown size={15} weight="bold"/></button></section>;
}
...
function CarRow({ car, navigate, favorite, toggleFavorite }) {
  ...
  const price = estimateLandedCost(car);
  return <article className="car-row" onClick={open} onKeyDown={(e) => (e.key === "Enter" || e.key === " ") && open()} tabIndex="0" role="button" aria-label={`Открыть ${car.title}`}><HoverImagePreview car={car} className="car-row-image"/><div className="car-row-info"><div className="row-title"><div><h2>{car.title}</h2></div><div className="row-actions"><button aria-label="Добавить в избранное" className={favorite ? "selected" : ""} onClick={(e) => {e.stopPropagation();toggleFavorite(car.id);}}><Heart size={21} weight={favorite ? "fill" : "regular"}/></button></div></div><p className="summary">{number(car.mileage)} км · {car.type} · {car.drive} привод</p><div className="mini-specs">{car.battery && <span><BatteryHigh size={17}/>{car.battery} кВт·ч</span>}{car.range && <span><Gauge size={17}/>{car.range} км</span>}<span><CarProfile size={17}/>{car.bodyType}</span></div><div className="source-line"><MapPin size={15}/>{translateCity(car.city)}<span>•</span><Clock size={15}/>Актуализировано {formatCheckedAt(car.checkedAt || car.importedAt)}</div></div><div className="car-row-price"><strong>≈ {money(price.totalUsd, currency)}</strong><span>Под ключ</span><b>{number(car.chinaPrice)} ¥</b><small>цена в Китае</small><button>Подробнее <ArrowRight size={16}/></button></div></article>;
}
...
const purchaseSteps = [
  { icon:MagnifyingGlass, title:"Вы выбираете автомобиль", text:"Смотрите реальные объявления, сравниваете комплектации и видите предварительную цену до Минска." },
  { icon:ChatCircleText, title:"Мы подтверждаем объявление", text:"Связываемся с продавцом, уточняем наличие, цену, VIN и возможность экспорта." },
  { icon:ShieldCheck, title:"Проверяем автомобиль", text:"Заказываем независимую диагностику кузова, техники и батареи. Результаты показываем до решения о покупке." },
  { icon:ListChecks, title:"Фиксируем смету", text:"Согласовываем автомобиль, логистику, таможенные платежи и услуги. Неподтверждённые суммы отмечаем отдельно." },
  { icon:CarProfile, title:"Выкупаем и доставляем", text:"Сопровождаем оплату, экспортные документы и перевозку. Вы получаете автомобиль в Минске." },
];

function HowItWorksPage({ navigate }) {
  return <main className="info-page">
    <section className="info-hero page-width"><div className="info-hero-copy"><button className="back-mobile" onClick={() => navigate("/")}><ArrowLeft size={18}/>На главную</button><span className="info-eyebrow">Как это работает</span><h1>Покупка авто из Китая — без прыжка в неизвестность</h1><p>Сначала проверка автомобиля и понятная смета. Только потом — решение о покупке, договор и оплата.</p><div className="info-actions"><button className="primary" onClick={() => navigate("/catalog")}>Выбрать автомобиль <ArrowRight size={18}/></button><a href="#steps">Посмотреть этапы</a></div></div><aside className="journey-preview" aria-label="Краткая схема покупки"><span>Ваш путь</span><div><b>01</b><p><strong>Выбор</strong><small>Каталог и расчёт</small></p><Check size={18} weight="bold"/></div><div><b>02</b><p><strong>Проверка</strong><small>Продавец, VIN, состояние</small></p><Clock size={18}/></div><div><b>03</b><p><strong>Доставка</strong><small>Документы и логистика</small></p><CarProfile size={18}/></div><p className="journey-note"><Info size={17}/> Деньги за автомобиль не переводятся до согласования результатов проверки и итоговой сметы.</p></aside></section>
    <section className="info-proof page-width"><div><ShieldCheck size={23}/><p><b>Проверка до оплаты</b><span>Сначала факты, затем решение</span></p></div><div><CurrencyCny size={23}/><p><b>Прозрачная стоимость</b><span>Разделяем цену и расчёт</span></p></div><div><MapPin size={23}/><p><b>Сопровождение до Минска</b><span>Один понятный маршрут</span></p></div></section>
    <section className="info-section page-width" id="steps"><div className="info-section-heading"><span>Пять этапов</span><h2>Что происходит после выбора автомобиля</h2><p>На каждом шаге вы понимаете, что уже подтверждено, что проверяется и за что платите.</p></div><div className="process-list">{purchaseSteps.map(({ icon:Icon, title, text }, index) => <article key={title}><div className="process-number">{String(index + 1).padStart(2,"0")}</div><div className="process-icon"><Icon size={24} weight="duotone"/></div><div><h3>{title}</h3><p>{text}</p></div></article>)}</div></section>
    <section className="decision-section"><div className="page-width decision-grid"><div><span className="info-eyebrow">До оформления</span><h2>Вы принимаете решение на основе полной картины</h2><p>Если автомобиль не проходит проверку или итоговые условия меняются, мы не подталкиваем к сделке — помогаем найти другой вариант.</p></div><div className="decision-card"><h3>До оплаты автомобиля вы получите</h3><ul><li><CheckCircle size={19} weight="fill"/>Подтверждение наличия и цены</li><li><CheckCircle size={19} weight="fill"/>VIN и результаты проверки</li><li><CheckCircle size={19} weight="fill"/>Итоговую смету с диапазонами</li><li><CheckCircle size={19} weight="fill"/>Понятный план доставки</li></ul></div></div></section>
...
function AboutPage({ navigate }) {
  const principles = [
    { icon:ListChecks, title:"Факты отдельно от оценки", text:"Показываем данные объявления, наши расчёты и то, что ещё требует подтверждения, как разные вещи." },
    { icon:ShieldCheck, title:"Проверка важнее обещаний", text:"Не называем автомобиль проверенным, пока нет подтверждения продавца и независимой диагностики." },
    { icon:Lightning, title:"Сложное — простым языком", text:"Переводим характеристики, документы и расходы в понятный формат без китайских обозначений и скрытых строк." },
  ];
  return <main className="info-page">
    <section className="about-hero page-width"><div><button className="back-mobile" onClick={() => navigate("/")}><ArrowLeft size={18}/>На главную</button><span className="info-eyebrow">О сервисе</span><h1>NaVostok помогает осознанно выбрать автомобиль из Китая</h1><p>Мы собираем объявления китайского вторичного рынка, приводим данные к понятному виду и сопровождаем путь от первой проверки до доставки в Минск.</p><button className="primary" onClick={() => navigate("/catalog")}>Открыть каталог <ArrowRight size={18}/></button></div><aside className="about-statement"><span>Наша роль</span><blockquote>Не просто показать объявление, а дать достаточно проверяемой информации для спокойного решения.</blockquote><small>Команда NaVostok.by</small></aside></section>
    <section className="info-section page-width"><div className="info-section-heading compact"><span>Наш подход</span><h2>Прозрачность на каждом шаге</h2></div><div className="principles-grid">{principles.map(({ icon:Icon, title, text }) => <article key={title}><span><Icon size={25} weight="duotone"/></span><h3>{title}</h3><p>{text}</p></article>)}</div></section>
    <section className="service-scope"><div className="page-width service-scope-grid"><div><span className="info-eyebrow">Что делает сервис</span><h2>Из объявления — в понятную карточку</h2><p>Мы структурируем параметры автомобиля, переводим данные, рассчитываем ориентир стоимости до Минска и показываем свежесть источника.</p></div><div className="scope-list"><div><b>01</b><p><strong>Собираем</strong><span>Характеристики, фото и данные продавца</span></p></div><div><b>02</b><p><strong>Объясняем</strong><span>Состояние, комплектацию и структуру цены</span></p></div><div><b>03</b><p><strong>Проверяем</strong><span>Наличие, VIN, экспорт и техническое состояние</span></p></div><div><b>04</b><p><strong>Сопровождаем</strong><span>Выкуп, документы и доставку</span></p></div></div></div></section>
    <section className="honesty-section page-width"><div><span className="info-eyebrow">Важно</span><h2>Чего мы не обещаем</h2></div><div className="honesty-list"><p><X size={19} weight="bold"/><span><b>Не выдаём данные продавца за независимую проверку.</b> Всё, что ещё не подтверждено, прямо так и обозначено.</span></p><p><X size={19} weight="bold"/><span><b>Не фиксируем цену раньше времени.</b> Курс, логистика и таможенные параметры уточняются перед договором.</span></p><p><X size={19} weight="bold"/><span><b>Не подбираем «любой ценой».</b> Если вариант сомнительный, честный результат — отказаться от него.</span></p></div></section>
...
function InfoCta({ navigate, title, text }) {
  return <section className="info-cta page-width"><div><span>Каталог NaVostok.by</span><h2>{title}</h2><p>{text}</p></div><button className="primary" onClick={() => navigate("/catalog")}>Перейти к автомобилям <ArrowRight size={18}/></button></section>;
}
...
export function App() {
  ...
  const [favorites,setFavorites]=useState(new Set()); const [currency,setCurrency]=useState(() => window.localStorage.getItem("navostok-currency") === "BYN" ? "BYN" : "USD"); const [cars,setCars]=useState([]); const [catalogTotal,setCatalogTotal]=useState(0); const [importedAt,setImportedAt]=useState(null); const [apiMode,setApiMode]=useState(false); const [loading,setLoading]=useState(true); const [routeLoading,setRouteLoading]=useState(Boolean(targetId)); const [loadError,setLoadError]=useState(false);
  useEffect(() => { window.localStorage.setItem("navostok-currency", currency); }, [currency]);
  useEffect(() => {
    let cancelled = false;
    const load = async () => {
      try {
