...
export const TOOL_PAGES = Object.freeze([
  ...
  },
  {
    ...
    kind: "brands",
    name: "Марки из Китая",
    // В заголовке и значки, и названия: спрашивают и то и другое, причём чаще
    // значками («что за марка с таким значком»), а не названиями.
    h1: "Китайские марки автомобилей: значки, названия и кто чем владеет",
    ...
    lead: "Кто скрывается за значком, как читается название и какому концерну принадлежит марка.",
  },
  {
    path: "/range",
    kind: "range",
    name: "Реальный запас хода",
    // «Зимой» в заголовке обязательно: спрашивают почти всегда про зиму, а летом
    // паспортная цифра мало кого удивляет.
    h1: "Реальный запас хода электромобиля: сколько он проедет зимой",
    ...
    lead: "Паспортные километры измерены в тепличных условиях. Посчитайте, сколько останется в мороз и на трассе.",
  },
  {
    path: "/price-belarus",
    kind: "market",
    name: "Дешевле ли из Китая",
    h1: "Дешевле ли привезти авто из Китая, чем купить в Беларуси",
    ...
    lead: "Сравниваем цены на одинаковые машины: объявления белорусского рынка против нашей цены под ключ в Минске.",
  },
]);
...
export function customsExample() {
  ...
  return {
    ...
    columns: ["Объём двигателя", "Моложе 3 лет", "От 3 до 5 лет", "Старше 5 лет"],
...
export function dutyRateTables() {
  ...
  return [
    {
      title: "Машина моложе трёх лет: доля от стоимости",
    ...
    },
    {
      ...
      columns: ["Объём двигателя", "От 3 до 5 лет", "Старше 5 лет"],
      rows: from3to5.map((row, index) => [
        ccRange(from3to5, index),
        eur(row.rate),
        eur(over5[index].rate),
      ]),
      note: "Стоимость машины на пошлину уже не влияет — важен только мотор. Ступень от трёх до пяти лет самая выгодная, и поэтому в нашем каталоге нет бензиновых машин старше 2021 года. После пяти лет ставка примерно вдвое выше, и сверху добавляется НДС 20%.",
    },
  ];
}
