...
export function modelAutoText({ name, facts }) {
  ...
  paragraphs.push(
    "Цена в карточке — итог до Минска: автомобиль, доставка, таможенные платежи и сборы по действующим правилам ввоза. Наличие сверяется с площадкой-источником каждую ночь, проданные машины уходят из списка.",
  );
  return paragraphs;
}
...
export function modelFaq({ name, facts, review = null }) {
  ...
  if (total && !covered(own, /в наличии|сколько .*машин|есть ли/i)) {
    ...
    auto.push({
      ...
      a: `${cars(total)}${years ? ` ${years}` : ""}${finite(facts?.mileageMin) ? `, пробег от ${number(facts.mileageMin)} км` : ""}. Список обновляется каждую ночь по данным площадки-источника; сортировка и фильтры над списком считают только эту модель.`,
    });
  }
