 ...
 * модель, число машин, цена «от»). Один расчёт на двоих: сборка встраивает результат
...
export function homePopularModels(rows = []) {
  const entries = rows
    .map(({ brand, model, count, priceMin }) => {
      const review = MODEL_PAGES.find((page) => page.brand === brand && page.model === model);
      const price = Number(priceMin) || null;
      return { brand, path: modelLandingPath(brand, model), name: review?.name || carTitle(brand, model), count: Number(count) || 0, priceFrom: price ? Math.round(price / 50) * 50 : null };
    })
