...
//
//   ev        — фиды 7/5/6/3 (электромобили, подключаемые гибриды, гибриды)
//   petrol-a  ┐
...
export function feedsForShift(shift) {
  // Фид 3 (обычный гибрид) добавлен 07.09.2026: такая машина у нас числится
  // гибридом, а `shiftOfCar` отправляет гибриды в эту же смену — без фида в
  // списке её машины проверялись бы в ночь, когда их списки никто не листает.
  return shift === "ev" ? [7, 5, 6, 3] : [1];
}
