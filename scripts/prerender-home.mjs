...
// Главная содержит только одну разметку — ту же, которую видит и оживляет React.
// Прежний скрытый блок для поисковика давал второй H1, повторял навигацию и исчезал
// после загрузки скрипта. Поисковик и посетитель теперь получают один и тот же текст.
...
console.log(
  `[prerender] главная: разметка приложения ${kb(app.length)} КБ вместо заглушки, ` +
    `без отдельного скрытого SEO-блока, ` +
