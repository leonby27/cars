...
const keyFor = (brand, model, year, mileageMax) => JSON.stringify([brand, model, year, mileageMax]);
...
for await (const line of stream) {
  ...
  if (!row.eligible || row.isNew || row.onOrder || !row.brand || !row.model || !Number.isFinite(price) || price <= 0 || !Number.isFinite(mileage) || mileage < 0 || !Number.isInteger(year)) continue;
  listings += 1;
  for (const mileageMax of mileageLimits) {
    if (mileageMax !== null && mileage > mileageMax) continue;
    const key = keyFor(row.brand, row.model, year, mileageMax);
...
for (const [key, values] of buckets) {
  const [brand, model, year, mileageMax] = JSON.parse(key);
  values.sort((left, right) => left - right);
  const sum = values.reduce((total, value) => total + value, 0);
  brands[brand] ||= {};
  brands[brand][model] ||= {};
  brands[brand][model][year] ||= {};
  brands[brand][model][year][mileageMax === null ? "all" : String(mileageMax)] = {
    count: values.length,
    min: values[0],
...
const output = {
  ...
  scope: "Used passenger-car listings without New or On order badges",
  mileageLimits,
  listings,
  brands,
};
