...
const daily = Array.from({ length: 100 }, (_, i) => ({ day: new Date(Date.UTC(2026, 5, 1 + i)).toISOString().slice(0,10), visits: i }));
...
test("сегодня и вчера выбирают только одну точку по минской дате", () => {
  const days = [{ day:"2026-09-07", visits:2 }, { day:"2026-09-08", visits:3 }];
...
test("шкала имеет целые отметки, включает ноль и максимум; одиночная точка центрируется", () => {
  for (const visits of [0,1,3,19,103]) {
    const { points, ticks } = visitsChart([{ day:"2026-09-08",visits }], "today", now);
    assert.equal(points[0].x, 50);
    assert.ok(points[0].y >= 10 && points[0].y <= 90);
    assert.equal(ticks[0].value, 0);
    assert.ok(ticks.at(-1).value >= visits);
    assert.ok(ticks.every(tick => Number.isInteger(tick.value)));
  }
});

test("линии Яндекса и Google используют общую шкалу заходов", () => {
  const { points, ticks } = visitsChart([
    { day:"2026-09-07", visits:12, yandex:8, google:3 },
    { day:"2026-09-08", visits:20, yandex:11, google:6 },
  ], "7", now);
