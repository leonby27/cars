...
// Пометка «чужая страница» ставится маленьким скриптом прямо в странице: файл главной
// веб-сервер подкладывает и под адреса без своей страницы (/favorites, /login), и там
// до запуска приложения должна быть видна только шапка.
//
// Скрипт собирается шаблонной строкой, а в ней `\/` превращается в простую косую
// черту — из-за этого выражение однажды уехало в прод как `//+$/`: остаток строки стал
// комментарием, скрипт обрывался с ошибкой в консоли и пометку не ставил. Поэтому здесь
// не сверка текста, а настоящий разбор: код обязан быть исполнимым.
test("скрипт-пометка «чужая страница» в готовой главной разбирается без ошибок", async () => {
  const html = await prerender();
  const scripts = [...html.matchAll(/<script>([^<]*foreign-boot[^<]*)<\/script>/g)].map((match) => match[1]);
  assert.equal(scripts.length, 1, "скрипта-пометки в странице нет");
  // Бросит SyntaxError, если код оборван, — именно это и случилось в проде.
  assert.doesNotThrow(() => new Function(scripts[0]), "скрипт-пометка не разбирается");
  // И он действительно отрезает косые черты, а не считает их началом комментария.
