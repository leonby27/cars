...
// Главная защита: раздел не должен уехать на боевой сайт включённым по недосмотру.
// На локальной версии журнал виден всегда — проверяем именно боевое значение.
// Когда журнал будет готов, эту проверку меняют вместе с самим выключателем.
test("на боевом сайте журнал выключен, пока его не включили осознанно", () => {
  assert.equal(shippedFlag("BLOG_ENABLED"), false, "журнал включён для боевого сайта — если это намеренно, поправьте и эту проверку");
});
